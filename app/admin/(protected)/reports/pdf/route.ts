...
import { getPeriodLabel, getPeriodSlug, type ReportPeriodType } from "@/lib/reports/constants";
...
export async function GET(req: Request) {
  ...
  const periodType = (searchParams.get("type") ?? "month").toUpperCase() as ReportPeriodType;
  const year = Number(searchParams.get("year"));
  const period = periodType === "YEAR" ? 0 : Number(searchParams.get("period"));
